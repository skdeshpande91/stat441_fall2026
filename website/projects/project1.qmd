...
The project report and recorded presentation are due on **Friday October 9 at 12:00pm (noon)**. 
...
Each team will also record an 8--10 minute presentation that provides an overview of their analysis.
...
::: {.callout-tip}
## Quarto & Zoom
I **highly** recommend using Quarto to prepare your written report and recording your presentation with Zoom.
For your report, you can either render the document directly as a PDF. Or, you can first render it as an HTML file and save that as a PDF.
For your presentation, please ensure that the speaker's face is visible along with the slide contents.
:::


Each team will submit a PDF document containing their written report; an .mp4 file of their presentation recording; and a compressed archive (i.e., a `.zip` or `.tar.gz` file) containing the reproducibility materials (i.e., the Quarto document used to produce the written report and any external data used).
Note, if you downloaded data directly into your R session (e.g., by using functions or data provided by an existing package or by reading in publicly-hosted file), you do not need to include those data in your reproducibility materials.

Each team will be assigned a two-digit team number (`teamID`) that should appear in the file name of all submitted files.
Please name your report as `teamID_report.pdf`, your presentation as `teamID_presentation.mp4`, and your reproducibility material as `teamID_code.zip` or `teamID_code.tar.gz`.
So, if your team number is `00`, you should save your report as `00_report.pdf`, your presentation as `00_presentation.mp4`, and your reproducibility archive as `00_code.zip` or `00_code.tar.gz`. 


You will upload the files separately using the following links 

  * [Submission link for written reports](https://uwmadison.app.box.com/f/2f99f0322b1144e3a12b7e6d6cf55717)
  * [Submission link for presentation recordings](https://uwmadison.app.box.com/f/7b96448768c540c1915b3ccf0c6ec961)
  * [Submission link for reproducibility materials](https://uwmadison.app.box.com/f/9916c3b053914fecbfa4461abdf7a45e)

:::{#callout-important}
## NetID Credentials
You must sign into Box using your Wisconsin NetID credentials before you can upload your submission
:::

## Peer Reviews

You will be assigned three projects to review. Assignments will be announced at noon on the day the project is due.
Your reviews are due 1 week after the project deadline (i.e., Friday October 16 at 12:00pm).
For each assigned project, please complete the peer review survey at [this Qualtrics link](https://uwmadison.co1.qualtrics.com/jfe/form/SV_2h37bruTLPm8phA).
You will need to fill out the survey three separate times, one for each project.
...
The package [**nhflscrapr**](https://rentosaijo.github.io/nhlscraper/) provides functions for scraping play-by-play data from the National Hockey League (including the puck and player tracking sprites for all events).
...
The [**hoopR**](https://hoopr.sportsdataverse.org) includes functionality to scrape basketball shot location data.
...
If you pursue this option, you should build and compare at least 3 different models of shot selection. You should then select the "best" model using both qualitative and quantitative considerations.
...
In [Lecture 5](../lectures/lecture05.qmd), we estimated regularized APM using data from a single season.
Many analysts prefer to use data from multiple seasons (with recent data weighted more heavily than past data) to estimate APM and regularized APM. 
The function [`nba_possession_lineups`](https://hoopr.sportsdataverse.org/reference/nba_possession_lineups.html?q=stint#null) in the [**hoopR**](https://hoopr.sportsdataverse.org/index.html) package and the function [`wnba_possession_lineups`](https://wehoop.sportsdataverse.org/reference/wnba_possession_lineups.html?q=stint#null) can be used to scrape stint-level data from each game. 
You could consider fitting multi-season APM models for the NBA or WNBA.
It would be especially interesting to assess the overall predictive quality of (regularized) adjusted plus/minus models.
-->
...
An even more ambitious project would develop a version of WAR for batting performance.
