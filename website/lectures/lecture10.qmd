...
roster2024 <-
  nflfastR::load_rosters(seasons = 2024) |>
...
Among the receivers with the largest $\textrm{IPA}_{\textrm{YAC}, c}^{(C)}$ values, we see players like <!-- TO DO -->
```{r}
#| label: top-yac-iap
yac_receiver_effects |>
  dplyr::inner_join(y = roster2024 |> dplyr::select(gsis_id, full_name), by = "gsis_id") |>
...
We repeat this calculation for the IPA values arising for both types of running play.
```{r}
#| label: ipaa-run
...
#| fig-alt: Scatterplot of wins and scoring differential in the NFL
