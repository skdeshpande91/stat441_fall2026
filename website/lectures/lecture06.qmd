...
## Motivation: Credit Allocation in Baseball {#sec-motivation}
...
[^korea]: This was also the first MLB game ever played in South Korea!

  1. In the 3rd inning, with 2 outs and no runners on base, Ohtani singled into right field.
  2. In the 8th inning, with 1 out and runners on first and second base, Ohtani singled into left center field, driving in one run.

Which of these singles was more valuable to Dodgers? 
And how much of that value should be specifically credited to Ohtani?

Because the second single directly resulted in a run scoring, it is tempting to conclude that the second single was more valuable than the first.
...
[^steal]: In fact, Ohtani went on to steal second base during the next at-bat, further increasing his team's chances of scoring. 
...
In this lecture, we begin by computing the number of runs that a team can *expect* to score in the remainder of a half-inning (@sec-expected-runs).
Then, we introduce *run values* (@sec-run-value), a metric that combines changes in the number of a team expects to score and the number of runs actually scored in an at-bat.
We conclude by determining which batters created the most run value for their teams (@sec-batter-prod).
...
We will then begin [Lecture 8](lecture08.qmd) by distributing the *negative* of the run value created by the batting team between the pitcher and fielders involved in each at-bat.
...
## Tracking Data in Baseball {#sec-statcast}
 
### A Brief History {#sec-history}

On October 4, 2006, during an American League Division Series game between the Oakland Athletics and Minnesota Twins, the sports media company Sportvision[^sportvision] debuted PitchF/X[^fast], a system of cameras for tracking the position of every pitch as it traveled from the pitcher's hand to the batter.
After installing the system in every MLB ballpark, Sportvision began providing the data in real-time data to the MLB.
These data, combined with additional information recorded by an MLB Advanced Media employee, powered the popular [GameDay application](https://www.mlb.com/schedule/gameday) and were publicly available through the GameDay application programming interface (API) for many years.

[^fast]: See [this article by Mike Fast](https://tht.fangraphs.com/wp-content/uploads/sites/8/images/pitchfx_guide.bt.pdf) for more background

In 2017, PITCHf/x system was phased out replaced by the radar-based Trackman system, which was originally developed to track the trajectories of golf balls.
Trackman is part of the larger Statcast system, which additionally tracks the movement of all players on the field.
See [this New York Times article](https://www.nytimes.com/athletic/5627303/2024/07/10/mlb-statcast-10-year-anniversary/) for more about the history of Statcast.

[^sportvision]: Sportvision is perhaps most famous for the [yellow first down line that appears in American football broadcasts](https://smt.com/behind-the-nfls-yellow-first-down-line-and-whats-next-for-sports-tv/). They were eventually acquired by [SMT](https://smt.com).

### Accessing Statcast Data in R {#sec-statcast-query}

Major League Baseball hosts a [public-facing web interface](https://baseballsavant.mlb.com) for accessing Statcast data.
Using that interface, users can pull up data for individual players or about all pitches of a certain type.
Powering this website is an API, which allows software applications to connect to the underlying Statcast database.
The package [**sabRmetrics**](https://github.com/saberpowers/sabRmetrics) uses this API to scrape and preprocess these data.
We will also make use of some functions provided by the [**baseballr**](https://billpetti.github.io/baseballr/) package
You can install the package using the following code
```{r}
#| label: install-sabRmetrics
#| eval: false
#| echo: true
...
The package **sabRmetrics** provides a function, `download_baseballsavant()`] to download pitch-by-pitch data from [Baseball Savant](https://baseballsavant.mlb.com) from all games between user-specified starting and ending dates.
To scrape an entire season's worth of data, we will parallelize the function's operations, using code provided on the [package's GitHub repository](https://github.com/saberpowers/sabRmetrics#downloading-data); see these [notes on parallel computing](https://dept.stat.lsa.umich.edu/~jerrick/courses/stat506_f24/16-parallel-processing.html) from Josh Erickson at Michigan for more information about parallel computing in R.
...
if(n_cores >= 4){ #<2>
  cluster <- parallel::makeCluster(n_cores-2) #<2>
  raw_statcast2024 <-  #<3>
    sabRmetrics::download_baseballsavant( 
      start_date = "2024-01-01", 
      end_date = "2024-12-31", 
      cl = cluster) 
  parallel::stopCluster(cluster)  #<3>
} else{
  raw_statcast2024 <- 
    sabRmetrics::download_baseballsavant(
      start_date = "2024-01-01",
...
```
1. Most modern computers have multi-core processors. To run across multiple cores, we need to first detect the number of available cores. See this [blog post about detecting cores](https://www.r-bloggers.com/2022/12/please-avoid-detectcores-in-your-r-packages/) for more information about issues with the more common call `parallel:detectCores()`.
2. If we have at least 4 available cores, we'll create a parallel socket cluster that will use all but two of those. Otherwise, we'll have to run this on a single core, which can take a considerably longer time.
3. Closes the cluster since we no longer need it.
...
So, if you want to work with data from all available seasons (2008 to the present), you will need about 2.5GB of storage space on your computer.
:::

## Statcast Basics {#sec-statcast-basics}
```{r}
#| label: load-statcast
#| echo: false
#| message: false
...
In total, `raw_statcast2024` contains data for `{r} nrow(raw_statcast2024)` regular season pitches.
...
For balls in play, Statcast also records things like the launch speed and angle (`launch_speed`, `launch_angle`), the coordinates on the field where the ball landed (`hit_coord_x`) and (`hit_cord_y`), and the position of the first fielder to touch the ball (`hit_location`).
In [Lecture 8](lecture08.qmd), we will work with `hit_coord_x` and `hit_coord_y` to build a model that predicts the probability that each fielder makes an out on balls hit to a specific part of the field.
As a bit of a preview, here is a plot of all the `hit_coord_x` and `hit_coord_y` values.
...
#| fig-height: 4
#| fig-align: center
#| fig-cap: Locations of all balls in play during the 2024 MLB regular season
#| fig-alt: Scatterplot showing locations of all balls hit into play

par(mar = c(3,3,2,1), mgp = c(1.8, 0.5, 0))
plot(raw_statcast2024$hit_coord_x, raw_statcast2024$hit_coord_y, 
     xlab = "hc_x", ylab = "hc_y",
     pch = 16, cex = 0.2, col = rgb(0,0,0, 0.1))
```

### Adding Baserunner Information {#sec-add-baserunner}

The columns `pre_runnber_1b_id`, `pre_runnber_2b_id`, and `pre_runnber_3b_id` record who is on first, second, or third base at the beginning of each pitch (`NA` values indicate that nobody is on base).
Because these variable names are rather long, we'll rename them to the shorter `on_1b`, `on_2b`, and `on_3b`.

```{r}
#| label: rename-baserunner-variables
#| eval: true
#| echo: true
statcast2024 <-
  raw_statcast2024 |>
...
So, by pasting together the results of `1*(!is.na(on_1b))`, `1*(!is.na(on_2b))`, and `1*(!is.na(on_3b))` 
we can form the 3-digit binary string described above. 
...
To access this, we will use the function [`baseballr::chadwick_player_lu()`](https://billpetti.github.io/baseballr/reference/chadwick_player_lu.html) downloads the Chadwick database and stores it as a data table in R.
...
save(chadwick_players, file = "chadwick_players.RData") # <1>

player2024_id <- # <2>
  unique(
    c(statcast2024$batter_id, statcast2024$pitcher_id,
      statcast2024$on_1b, statcast2024$on_2b, statcast2024$on_3b,
      statcast2024$fielder_2_id, statcast2024$fielder_3_id,
      statcast2024$fielder_4_id, statcast2024$fielder_5_id, 
      statcast2024$fielder_6_id, statcast2024$fielder_7_id, 
      statcast2024$fielder_8_id, statcast2024$fielder_9_id))
...
save(player2024_lookup, file = "mlb_player2024_lookup.RData")
```
1. Save a local copy of the full Chadwick database.
2. This vector contains the MLB Advanced Media ID for all players who appeared in 2024 as a batter, pitcher, fielder, or baserunner.
...
### Getting Player Positions {#sec-positions}
...
baseballr::mlb_batting_orders(game_pk = 745444) #<1>
```
1. **sabRmetrics** uses variable name `game_id` and **baseballr** uses the variable name `game_pk` to refer to MLBAM's numeric identifier for each game.
...
#| eval: false
all_lineups <- list()
unik_game_pk <- unique(statcast2024$game_id)
...
lineups2024 <- 
  ...
  unique()
save(lineups2024, file = "mlb_lineups2024.RData") # <3>
```
...
save(positions2024, file = "mlb_positions2024.RData")
```
...
## Expected Runs {#sec-expected-runs}
...
### Computing Runs Scored in the Half-Inning {#sec-compute-runs-scored}
...
dodgers_inning <-
  statcast2024 |>
  dplyr::filter(game_id == 745444 & inning == 8 & half_inning == "top") |>
  dplyr::select(
    at_bat_number, pitch_number, outs, BaseRunner,
    bat_score, post_bat_score, events, description, des,
    type, on_1b, on_2b, on_3b, hit_coord_x, hit_coord_y, hit_location) |>
...
![Play-by-Play from the March 20, 2024 Dodgers-Padres game](figures/lecture06/dodgers_pbp.png){fig-align="center" fig-alt="Screenshot of play-by-play data"#fig-dodgers-pbp}
...
### Putting It All Together {#sec-runs-remaining}

We're now ready to extend these calculation to every half-inning of every game in the season.
To do this, we will take advantage of the `dplyr::group_by()` function to apply the same calculation to subsets defined by game and half-inning.
```{r}
...
We can re-format `expected_runs` to this matrix format using the [`tidyr::pivot_wider()`](https://tidyr.tidyverse.org/reference/pivot_wider.html) function
```{r}
#| label: pivot-rho
expected_runs |> 
  tidyr::pivot_wider(
...
### Calculating Runs Score {#sec-compute-runsscored}
...
To compute the number of runs scored within each at-bat, we will:
  ...
  2. Take the difference between the *last* value of `post_bat_score` and *first* value of `bat_score` within each at-bat.
...
We can now apply this formula to all pitches in `statcast2024` by grouping by `game_id` and `at_bat_number`.
...
statcast2024 <-
  statcast2024 |>
  dplyr::group_by(game_id, at_bat_number) |> # <1>
...
### Computing Starting & Ending States {#sec-computing-states}
...
runValue2024 <-
  ...
  dplyr::mutate(
    ...
    next_BaseRunner = dplyr::lead(BaseRunner)) |>
  dplyr::ungroup() |>
  dplyr::group_by(game_id, at_bat_number) |>
  dplyr::arrange(pitch_number) |>
  dplyr::mutate(
...
### Computing Run Values {#sec-compute-run-values}
...
In particular, we can use a join to add in the values of the starting and ending expected runs.
...
#| label: atbat-dodgers
runValue2024 |>
  dplyr::filter(game_id == 745444 & inning == 8 & inning_topbot == "Top") |>
...
runValue2024 <-
  ...
  dplyr::select(game_id, at_bat_number, RunValue)

rm(end_expected_runs)
save(runValue2024, file = "mlb_runValue2024.RData")
```

## Assessing Batter Production {#sec-batter-prod}
...
To do this, we first look up Ohtani's MLB Advanced Media ID number using our table `player2024_lookup`.
...
```{r}
#| label: load-lookup
load("mlb_player2024_lookup.RData")
```
...
ohtani_ab <-
  statcast2024 |>
  dplyr::filter(game_id == 745444) |>
  dplyr::filter(pitch_number == 1 & batter_id == ohtani_id) |>
  dplyr::select(game_id, at_bat_number, inning, des) |>
...
## Exercises
  ...
  1. Scrape data from the 2023 and 2024 regular seasons and determine which batters created the most run value.
  2. Compute a new version of expected runs that conditions on the number of outs, baserunner configuration, and ballpark. Using this new version of expected runs, determine which batters create the most run value for their team. Does ballpark appear to make much difference?
-->