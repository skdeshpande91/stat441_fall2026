...
In this lecture, we introduce *expected points* (EP; @sec-ep) and expected points added (EPA), which together facilitate a more nuanced, *quantitative* comparison of plays.
Then, using a multilevel model (@sec-multilevel-model), we will identify the players whom we would expect to generate the most expected points per passing attempt. 

## Play-By-Play Football Data {#sec-nflfastr-intro}

We will use play-by-play data from the NFL available from the [**nflfastR**](https://www.nflfastr.com) and [**nflreadr**](https://nflreadr.nflverse.com/index.html) packages, which can be installed using the code:
```{r}
#| label: install-nflfastR
#| eval: false
install.packages(c("nflfastR", "nflreadr"))
```
...
We can load a table containing roster information, including player identifier, name, position, and team using the function [`nflreadr::load_rosters()`](https://nflreadr.nflverse.com/reference/load_rosters.html)
```{r}
#| label: load-roster
#| warning: false
#| message: true
roster2024 <-
  nflreadr::load_rosters(seasons = 2024)
```

## Expected Points {#sec-ep}
...
We collect these probabilities into a vector $\boldsymbol{\pi}(\boldsymbol{\mathbf{z}}) = (\pi_{\textrm{TD}}(\boldsymbol{\mathbf{z}}), \ldots, \pi_{\textrm{oFG}}(\boldsymbol{\mathbf{z}})).$  
:::
...
### Estimating $\textrm{EP}(\boldsymbol{\mathbf{z}})$ {#sec-estimating-ep}
...
### Basic EPA Uses {#sec-basic-usage}
...
### Player-Specific EPA/Pass {#sec-empirical-average}
...
```{r}
#| label: build-alpha
names(ols_betas)[1] <- paste0("passer_player_id", rodgers_id) #<1>
names(ols_betas) <- 
...
### Partial Pooling {#sec-partial-pooling-motivation}
...
We can fit multilevel models using the [**lme4** package](https://cran.r-project.org/web/packages/lme4/index.html), which can be installed using the code
```{r}
#| label: install-lme4
#| eval: false
...
#| fig-asp: 0.5625
#| fig-align: center
#| fig-cap: Although our multilevel model heavily shrink the raw per-pass EPA values (black dots) towards the overall sample mean (blue line), there is still player-to-player variability in the resulting estimates (orange).
...
\end{align}
$$
Like our original multilevel model, Level 2 of our new model assumes that each player's latent per-pass EPA is normally distributed around some grand mean $\alpha_{0}.$
...
[^confusion]: Personally, I find the "fixed effects" and "random effects" terminology unhelpful. For one thing, the terms mean [different things to different people](https://statmodeling.stat.columbia.edu/2005/01/25/why_i_dont_use/)! And, since I mostly work within the Bayesian paradigm, in which we use probability to model *all* of our uncertainty and not just to random noise or sampling variable, I could argue that everything is a "random." I find it much more useful to think carefully about whether a covariate's effect might vary across groups or whether it could be assumed to be constant. But, since you will likely see this terms in future courses or later in your careers, we'll use them.
...
#| fig-asp: 0.5625
#| fig-align: center
#| fig-cap: Even after adjusting for covariate effects, players with fewer passing attempts display much more variation in per-pass EPA than players with more passing attempts.
...
We will use the model estimates to compute a version of wins above replacement for offensive players in the NFL.
