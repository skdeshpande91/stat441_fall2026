...
We will use play-by-play data from the NFL available from the [**nflfastR**](https://www.nflfastr.com) package, which can be installed using the code:
```{r}
...
We can load a table containing roster information, including player identifier, name, position, and team using the function [`nflfastR::load_roster()`](https://nflreadr.nflverse.com/reference/load_rosters.html)
```{r}
#| label: load-roster
#| warning: false
#| message: true
roster2024 <-
  nflfastR::load_roster(seasons = 2024)
```
