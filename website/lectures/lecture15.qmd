---
title: "Lecture 15: Markov Chains I"
format: html
execute: 
  cache: true
---
...
And how unusual is it to reach the state `0.111` from the state `0.110`? 

In this lecture, we will build a **Markov chain** model, which is a simple probabilistic model of state transitions, to answer such questions.
...
Finally, in @sec-absorbing, we will lean on some mathematical properties of *absorbing Markov chains* to estimate how many more at-bats a team can expect to face given the current game state.


### Data Preparation {#sec-data-prep}

The first at-bat in any half-inning of baseball has a deterministic starting state: `0.000`, with no outs and no runners on base.
...
So, if a team were to play the half-inning over and over again, the exact number of states visited (i.e., at-bats) and the precise sequence might vary from repetition to repetition.
Markov chains offer a conceptually simple way to model the randomness of the sequence of game states visited in a half-inning. 

To build a Markov chain model for baseball, we need to record the starting and ending state of every at-bat in the 2024 regular season.
Recall that in our data table `atbat2024,` the variable `GameState` records the value of $G_{t}$ and was formed by concatenating the variables `Outs` and `BaseRunner`.
...
Let us denote the game state at the start of at-bat $t$ of a half-inning as $G_{t}.$
...
Once we do this, we could compute things like the probability of a half-inning lasting at least 5 at-bats by finding all sequences of length 5 or more and adding up the corresponding probabilities.
Similarly, if we wanted to know the probability that the half-inning began by visiting the states `0.000`, `0.100`, `0.110`, and `0.111`, we would find all sequences with that starting sub-sequence and add up the corresponding probabilities.

Of course, calculating the probability for every possible sequence of every possible length is practically impossible.
Luckily, it turns out that the probability distribution of a (possibly infinite) sequence of random variables $\{G_{t}\}$ is fully determined by the sequence of *conditional* distributions of the form $G_{t} \vert G_{t-1}, \ldots, G_{1}.$
...
A sequence of random variables $\left\{X_{t}\right\}_{n = 1}^{\infty}$ is called a **Markov chain** if for all $t \geq 1$, all sets $A,$ and trajectories $x_{1}, \ldots, x_{t-1}$
$$
...
When the set of states is discrete (e.g., game states in baseball), Markov chains are fully characterized by a **transition probability** matrix.
...
Further suppose that 
  ...
  * When the system is in state `1`, it can remain there with probability 0.6 and it can move to state `2` with probability 0.4.
  * When the system is in state `2`, it remains there with probability 0.5 and it moves to state `1` with probability 0.5.
...
set.seed(479) 
states <- c(1,2) #<1>
transition_matrix <- matrix(c(0.6, 0.4, 0.5, 0.5), nrow = 2, ncol = 2, byrow = TRUE) #<2>
...
We can visualize transition probability matrix $\boldsymbol{\mathbf{P}}$ with a *directed graph* whose vertices correspond to the distinct states and whose edges are weighted by the corresponding transition probabilities.
In the graph, the edge from vertex $s$ to $s'$ is the assigned a weight of $p_{s,s'},$ which is the $(s,s')$ entry of $\boldsymbol{\mathbf{P}}$ and equals the probability of moving to state $s'$ from state $s.$
@fig-twostate-graph illustrates the graph corresponding to the simple two-state Markov chain from before.


![Graphical representation of the transition probabilities in a two-state Markov chain with one absorbing state](figures/lecture15/two-state-graph.png){#fig-twostate-graph width=50% fig-align="center"}


We can interpret a Markov chain with transition matrix $\boldsymbol{\mathbf{P}}$ as a *random walk* along this graph.
Whenever the walk is at vertex $s$, it picks one of the outgoing edges from $s$ to follow at a random.
The probability that the walk follows the edge from $s$ to $s'$ 
...
@fig-threestate-graph is a graphical illustration of this transition matrix.
Notice that the absorbing state `3` only has one outgoing edge that (i) leads back to itself and (ii) has an edge weight of one.

![Graphical representation of the transition probabilities in a three-state Markov chain with one absorbing state](figures/lecture15/three-state-graph.png){#fig-threestate-graph width=50% fig-align="center"}
...
Tabulating the different values in `absorption_time`, we see that the chain
  ...
  2. Reached state `3` in exactly two steps (so $T^{A}_{1} = 2$) in about 3200 of the 10,000 simulations
...
The fundamental feature of Markov chains is that the state the chain visits next depends only on its current state and not on any of the previously visited states.
...
In a baseball context, this would mean that what happens after the game reaches any particular state does not depend on what ultimately lead to the current state.
...
It is important to note, we are **assuming** that the Markov property holds.



Ultimately, we must estimate the transition probabilities between every pair of the 24 non-absorbing states and the absorbing state corresponding to the end of the half-inning.
...
@fig-baseball-graph shows a graphical representation of the transition matrix between the game states in baseball.
In the figure, the color and thickness of each edge between different states is proportional to corresponding edge transition probabilities, with colors range from blue (0%) to red (100%).
So that they are more readily apparent, we have highlighted the self-edges. 



```{r}
#| label: fig-baseball-graph
#| eval: true
#| echo: false
#| fig-width: 8
#| fig-height: 8
#| fig-cap: "Graphical representation of transitions between game states in baseball"
#| fig-align: center
#| fig-alt: "Graph with nodes corresponding to baseball game states. Edge colors correspond to transition probability."
...
support <- which(transition_matrix != 0, arr.ind = TRUE, useNames = TRUE)


colnames(support) <- c("from", "to")
edge_mat <-
  support |> 
  as.data.frame() |> 
  dplyr::mutate(Start = rownames(transition_matrix)[from], 
                End = colnames(transition_matrix)[to]) |>
  dplyr::select(Start, End) |>
  dplyr::rowwise() |>
  dplyr::mutate(weight = transition_matrix[Start, End])
rownames(edge_mat) <- NULL

g <- igraph::graph_from_edgelist(edge_mat[,1:2] |> as.matrix())
tmp_layout <- layout_in_circle(g)

v_color <- rep(adjustcolor(oi_colors[9], alpha.f = 0.2), times = 25)
names(v_color) <- colnames(transition_matrix)
v_color["3.000"] <- adjustcolor(oi_colors[8], alpha.f = 0.8)

E(g)$weight <- edge_mat$weight
V(g)$color <- v_color
edge_color <- adjustcolor(rgb(colorRamp(col_list,bias=1)(E(g)$weight)/255),
            alpha.f = 0.5)
E(g)$color <- edge_color

E <- t(apply(as_edgelist(g),1,sort))
E(g)$curved <- 0
E(g)[duplicated(E) | duplicated(E,fromLast =TRUE)]$curved <- 0.8
V(g)$label.cex <- 1.2

arrow_size <- 2.5*edge_mat$weight
arrow_size[edge_mat$weight < 0.1] <- 0.25


line_width <- 5 * edge_mat$weight
line_width[edge_mat$weight < 0.1] <- 0.5
line_width[edge_mat$Start == edge_mat$End & edge_mat$weight < 0.5] <- 1

plot(g, layout = tmp_layout, vertex.size = 20, vertex.label.color = oi_colors[1],
     edge.arrow.size = arrow_size, edge.width = line_width, xpd = TRUE)
```
...
```
We find, for instance, that in 5 of the 10 simulations, the pitching team retired the first three batters, none of whom got on base.
In Simulation 5, the last at-bat began in state `1.100` and ended in `3.000`, which means that the last at-bat involved an inning-ending double play.
...
table(inning_length)
```
In `{r} sum(inning_length == 3)` of our 50,000 simulations, the inning ended after exactly 3 at-bats.
And in `{r} sum(inning_length <= 6)` the inning ended with 6 or fewer at-bats.
Interestingly, there is one simulations in which the half-inning included 15 at-bats.
...
We can now tabulate the number of runs scored in each simulated half-inning

```{r}
#| label: runs-scored-tabulation
#| eval: true
#| echo: true
table(runs_scored)
...
## Modeling Multiple Steps {#sec-nstep}

The $(s,s')$ entry of $\boldsymbol{\mathbf{P}}$ is the probability that the chain moves from state $s$ to state $s'.$
Based on the 2024 MLB season, we estimated that approximately `{r} paste0(round(100*transition_matrix["1.000", "1.100"], digits = 2),"%")` of at-bats that began in the state `1.000` ended in the state `1.100`, so $p_{\textrm{1.000, 1.100}} \approx 0.23$ 
```{r}
...
We find that there is roughly a `{r} paste0(round(100*P2["1.000", "2.100"], digits = 2),"%")`  probability of reaching `2.100` from `1.000`
...
```
Starting from `0.000`, there is about a `{r} paste0(round(100*P2["0.000", "2.000"]), "%")` chance that the game reaches the state `2.000` after two at-bats[^intuition].
Similarly, there is about a `{r} paste0(round(100*P2["1.000", "3.000"]),"%")` chance that the inning ends within two batters of an at-bat beginning in state `1.000`.
Taking things one step further, we can compute the three-step transition probability matrix:
...
## Theory of Absorbing Markov Chains {#sec-absorbing} 
...
Eventually all half-innings ends, reaching the absorbing state `3.000`.

Now, suppose we have an absorbing Markov chain defined over the states $\{1, 2, \ldots, S\}.$
Without losing any generality, let us suppose that the first $T$ states are non-absorbing and the last $A = S - T$ states are absorbing.
Then, the transition matrix has the form
$$
\boldsymbol{\mathbf{P}} = \begin{pmatrix} Q & R \\ 0 & I_{A} \end{pmatrix},
$$
where $Q$ is a $T \times T$ matrix capturing one-step transition probabilities between non-absorbing states; $R$ is a $T \times A$ matrix capturing one-step transition probabilities between non-absorbing and absorbing states; and $I_{A}$ is the $A \times A$ identity matrix, with ones along the diagonal and zeros in all other entries.

In our baseball example, there is only $A = 1$ absorbing state and $T = 24$ non-absorbing states.
...
We also observe that the diagonal entries all appear to be larger than 1.
```{r}
