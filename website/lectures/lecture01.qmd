...
In this note, we will practice using functions from the **tidyverse** suite of packages (especially **dplyr**) to manipulate tables of NBA box score data.
...
## Basic Box Score Statistics {#sec-basic-box-score}

We will use the package [**hoopR**](https://hoopr.sportsdataverse.org/index.html) to scrape NBA boxscore data.
You should install the package using the code 
```{r}
#| label: install-hoopR
#| eval: false
install.packages("hoopR")
```
The function [`hoopr::load_nba_player_box()`](https://hoopr.sportsdataverse.org/reference/load_nba_player_box.html) loads season-level box-score data:
```{r}
#| label: scrape-boxscores
#| output: false
#| message: false
#| warning: false
...
Since we're ultimately interested in identifying the best player-seasons in terms of shooting performance, we need to remove all play-off, play-in, and All-Star games from the data.
...
reg_box <-
  ...
  dplyr::mutate(
    FGM = ifelse(is.na(minutes), 0, FGM), # <2>
  ...
  tidyr::replace_na(list(minutes = 0))                # <3>  
```
...
This functionality is implemented using [`dplyr::group_by()`](https://dplyr.tidyverse.org/reference/group_by.html)

```{r}
#| label: get-season-totals
season_box <-
  reg_box |>
...
## From Totals to Percentages {#sec-percentages}
...
We can identify this player-season by sorting the data in `season_box` by `FGM` in descending order with the [`dplyr::arrange()`](https://dplyr.tidyverse.org/reference/arrange.html) function
```{r}
#| label: arrange-fgm
season_box |>
...
On the basis of made shots alone, it is tempting to conclude that James Harden's 2018-19 season is more impressive than Giannis Antetokounmpo's 2023-24 season; after all, Harden made 6 more shots than Giannis.
But taking a closer look, we see that Harden took 540 more to make 6 more baskets than Giannis.
Arguably, Gianni's 2023-24 season is more impressive because he was more *efficient*. 
...
season_box <-
  season_box |>
  dplyr::mutate(FGP = ifelse(FGA > 0, FGM/FGA, NA_real_)) # <1>
season_box |> 
...
### Effective Field Goal Percentage {#sec-efgp}
