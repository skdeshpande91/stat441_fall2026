...
#| fig-asp: 0.5625
#| fig-align: center
#| fig-cap: Out-of-sample error plotted against regularization parameter (lambda)
#| fig-alt: Plot of out-of-sample error plotted against regularizaiton parameter
...
```{r}
#| label: luka-ad-pre-compute
#| eval: true
#| echo: false
luka_rapm <- rapm |> dplyr::filter(Name == "Luka Doncic") |> dplyr::pull(rapm)
ad_rapm <- rapm |> dplyr::filter(Name == "Anthony Davis") |> dplyr::pull(rapm)
```
For instance, by replacing Luka Dončić with Anthony Davis, teams are expected to score an additional `{r} round(ad_rapm - luka_rapm, digits = 1)` points per possession[^nico].
...
As a concrete illustration, the following code computes 500 bootstrap estimates of the RAPM parameter vector.
...
5. Use the players name as column names for `boot_rapm`
...
#| fig-alt: Histogram of bootstrap samples of difference between Doncic & Davis

boot_diff <- boot_rapm[,"Anthony Davis"] - boot_rapm[,"Luka Doncic"] #<1>
...
hist(boot_diff, #<2>
     ...
     xlab = "Difference")
```
1. Get the vectors of Dončić's and Davis' bootstrapped RAPM values by extracting the relevant columns from `boot_rapm` and compute the difference
2. Make a histogram of the bootstrapped differences between Davis' and Dončić's RAPM values
3. Set the number of bins in the histogram to 20
...
```{r}
#| label: sga-boot-table
#| eval: true
#| echo: true
sga_ranks <- boot_rank["Shai Gilgeous-Alexander",]
table(sga_ranks)
```
