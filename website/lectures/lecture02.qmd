...
Below are links to videos for three of her goals. Which is most impressive to you? 
  ...
  * [Goal against Austria](https://youtube.com/clip/UgkxnOKsnw4W3h-BqV5kyh-tOVnnTgBEc9u5?si=zpQB7OF43gt3yiwK)
  * [Goal against Norway](https://youtube.com/clip/UgkxqZ03bRBGCr4vS0M4yTMtHMOHsA6kkTHw?si=2IlC7AVi2CQotunG)
  * [Goal against Sweden](https://youtube.com/clip/UgkxwW9N1bXUv4G9HoSHVvYy_rk_aSjkrgwO?si=O90nkbgNd2FTv3e_)
...
We can access this data directly in R using the [**StatsBombR**](https://github.com/statsbomb/StatsBombR) package, whih you can install using the code below:
...
![Screenshot of the competitions covered in the public data](figures/lecture02/statsbomb_freecomps.png){width=100% fig-align="center" #fig-statsbomb-freecomps fig-alt="Table showing the competitions available in the free data provided by StatsBomb"}
...
For instance, here are the first few rows and some selected columns from the matches from EURO 2022.
StatsBomb graciously provided data for all the matches in the tournament.
Here is a table of matches from the 2022 EURO Competition; StatsBomb graciously provided data for all matches from the tournament, which can be obtained using the code below.
```{r}
...
The only women's internationals matches for which StatsBomb data were from the 2019 and 2023 World Cup and the 2022 and 2025 EURO tournaments.
...
Then, for every left-footed shot Mead attempted, we want to copy over the corresponding value from the table `xg_model1`, which in this case is 0.114.
...
mead_shots <-
  ...
  dplyr::left_join(y = xg_model1 |> dplyr::select(shot.body_part.name, XG1), # <1>
                   by = c("shot.body_part.name"))
```
1. No need to include `n` when joining the tables
...
These turn out to be in rows 1, 4, and 14 of the table `mead_shots`
```{r}
...
```
According to our first model, the "impressiveness" of these goals is pretty similar: our model put the respective chances of each shot resulting in a goal at about 11%.
...
[^cromwell]: Indeed, it seems absurd to claim that, at least in women's international soccer, players will **never** score off left-footed lobs! As the Statistician Dennis Lindley put it, we must "never believe in anything absolutely" and we should "leave a little probability for the moon being made of green cheese; it can be as small as 1 in a million, but have it there since otherwise an army of astronauts returning with samples of the said cheese will leave you unmoved" [@Lindley_decision, Section 6.7; here is the [fulltext](https://archive.org/details/makingdecisions00dvli/page/104/mode/2up) of the article].Lindley termed this principle ["Cromwell's Rule"](https://en.wikipedia.org/wiki/Cromwell%27s_rule), a reference to Oliver Cromwell's quote "I beseech you, in the bowels of Christ, think it possible that you may be mistaken" from his [letter to the Church of Scotland](https://www.olivercromwell.org/Letters_and_speeches/letters/Letter_129.pdf).
