...
In [Lecture 6](lecture06.qmd#sec-run-value), we computed the run value created by the offensive team in each at-bat of the 2024 MLB regular season. 
...
We then [ranked players](lecture06.qmd#sec-batter-prod) based on their run value totals, aggregating over all their at-bats.
...
### Conservation of Runs {#sec-conservation}
...
In this lecture, we will apportion $\delta_{i}$ between the batters (@sec-baserunner) and the baserunners involved in at-bat $i$ (@sec-batting)
In [Lecture 8](lecture08.qmd), we will apportion $-\delta_{i}$ between the pitcher and fielders involved in at-bat $i.$

## Data Preparation {#sec-data-prep}

To divide up offensive run value, we need to create a data table whose rows correspond to individual at-bats.
...
raw_atbat2024 <- 
  ...
  dplyr::select(game_date, game_id, at_bat_number, inning, inning_topbot,
         outs, BaseRunner, batter_id, on_1b, on_2b, on_3b, bat_score, fld_score, 
...
## Adjusted Run Values {#sec-adjusted-rv}
...
atbat2024 <-
  raw_atbat2024 |>
  dplyr::filter(!is.na(end_events) & !is.na(end_des)) |> #<1>
  dplyr::mutate(GameState = paste(outs, BaseRunner, sep = "."))
```
1. Drop all rows that are missing an `end_events` or `end_des`. Overwhelmingly, these are special situations (e.g., a truncated plate appearance)
...
Instead of "binning and averaging", like we did with our [distance-based XG models in Lecture 3](lecture03.qmd#sec-logistic), we will fit a statistical model.
A natural starting model asserts that there are numbers $\alpha_{0.000}, \ldots, \alpha_{2.111}$ and $\alpha_{\textrm{catcher\_interf}}, \ldots, \alpha_{\textrm{walk}}$ such that for all game states $\textrm{g}$ and ending events $\textrm{e},$
$$
...
We estimate $\hat{\alpha}_{2.000} \approx 0.359$ and $\hat{\alpha}_{single} \approx 0.111.$ 
So, according to our fitted model the average run value created by singling when there are two outs and no runners on is about $0.47.$
...
save(atbat2024, file = "mlb_atbat2024.RData")
```


## Baserunning Run Value {#sec-baserunner}
...
### Baserunner Advancement {#sec-baserunner-advancement}

Unfortunately, StatCast does not compute the number of bases that each runner advances during each at-bat.
The following code implements a function that determines the number of bases advanced by the runner on first (if any).
It works by first checking whether there is anyone on 1b at the start of the at-bat.
If so, it checks whether that player is on first, second, or third base at the end of the at-bat.
If not, it parses the at-bat description contained in `des` and looks for a sentence containing the player's name.
If that sentence contains the words "out" or "caught stealing", it sets the number of bases advanced to 0.
But, if the sentence contains the word "score", it sets the number of bases advanced to 3, since the runner scored from first.
```{r}
load("mlb_player2024_lookup.RData")
...
#| label: mvt-functions
#| code-fold: true
#| code-summary: Show code for computing the number of bases advanced by the batter and the runners on 2nd and 3rd base.
...
mvt_batter <- function(batter_id, outs, bat_score, end_on_1b, end_on_2b, end_on_3b, end_Outs, end_bat_score, des)
{
  mvt <- NA
  if(!is.na(end_on_1b) & batter_id == end_on_1b) mvt <- 1 # batter advanced to 1st
  else if(!is.na(end_on_2b) & batter_id == end_on_2b) mvt <- 2 # batter advanced to 2nd
  else if(!is.na(end_on_3b) & batter_id == end_on_3b) mvt <- 3 # batter advanced to 3rd
  else{
    # batter is not on base
    # look up player name
    player_name <- player2024_lookup$Name[which(player2024_lookup$key_mlbam == batter_id)]
...
baserunning <-
  ...
  dplyr::rowwise() |>
  dplyr::mutate(
    mvt_batter = mvt_batter(batter_id, outs, bat_score, end_on_1b, end_on_2b, end_on_3b, end_Outs, end_bat_score, end_des),
    mvt_1b = mvt_1b(on_1b, outs, bat_score, end_on_1b, end_on_2b, end_on_3b,end_Outs, end_bat_score, end_des),
    mvt_2b = mvt_2b(on_2b, outs, bat_score, end_on_2b, end_on_3b, end_Outs, end_bat_score, end_des),
    mvt_3b = mvt_3b(on_3b, outs, bat_score, end_on_3b, end_Outs, end_bat_score, end_des)) |>
  dplyr::ungroup() 
```

### Cumulative Baserunning Probabilities {#sec-baserunning-prob}
...
### Baserunning Runs Above Average {#sec-baserunner-raa}
...
baserunning |>
  dplyr::filter(game_id == 745444 & batter_id == ohtani_id) |>
  dplyr::select(at_bat_number, mvt_batter, end_des)
```
...
#| code-fold: true
#| code-summary: Compute the baserunning runs above average created by batter and the runners on second and third bases
raa_2b <-
  baserunning |>
  dplyr::filter(!is.na(on_2b)) |>
...
raa_batter <-
  ...
  dplyr::rename(key_mlbam = batter_id)
```
...
raa_br |>
  dplyr::arrange(dplyr::desc(RAA_br)) |>
  dplyr::slice_head(n = 5)
```

Interestingly, the player with the largest $\textrm{RAA}^{\textrm{br}}$, Makiel Garcia was an exceptionally good baserunner in the 2024 season[^garcia].

[^garcia]: See [this FanGraph blog](https://blogs.fangraphs.com/maikel-garcia-is-leveling-up/) that mentions his baserunning.

## Crediting Batters {#sec-batting}

We will credit $\hat{\mu}_{i}$ to each batter.
The following code block totals the $\hat{\mu}_{i}$ for each batter.
```{r}
#| label: compute-raa-b
batting <- 
  atbat2024 |>
  dplyr::select(batter_id, mu) |>
  dplyr::rename(key_mlbam = batter_id)
...
save(raa_br, raa_b, file = "mlb_raa_offensive2024.RData")
```
