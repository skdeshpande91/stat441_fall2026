...
Recall from [Lecture 6](lecture06.qmd) that the Statcast variables `hit_coord_x` and `hit_coord_y` record the coordinates where each batted ball is first fielded.
When we plotted the available `hit_coord_x` and `hit_coord_y` values, we saw the outlines of the park, with home plate located around `hit_coord_x = 125` and `hit_coord_y = 200.`
...
#| fig-alt: Scatterplot of batted ball hit coordinates

par(mar = c(3,3,2,1), mgp = c(1.8, 0.5, 0))
plot(statcast2024$hit_coord_x, statcast2024$hit_coord_y, 
     xlab = "hit_coord_x", ylab = "hit_coord_y",
     pch = 16, cex = 0.2, 
...
When we plot just the pitches first fielded by first basemen (`hit_location=3`), we see that in the original `(hit_coord_x, hit_coord_y)` coordinate system, first base is on the *right* hand side of the plot.
```{r}
#| label: fig-fielder3
#| fig-width: 6
#| fig-height: 6
...
#| figt-alt: Scatterplot of batted ball locations fielded by the first baseman

par(mar = c(3,3,2,1), mgp = c(1.8, 0.5, 0))
plot(statcast2024$hit_coord_x[statcast2024$hit_location!=3], 
     statcast2024$hit_coord_y[statcast2024$hit_location != 3], 
     xlab = "hit_coord_x", ylab = "hit_coord_y",
     pch = 16, cex = 0.2, 
     col = adjustcolor(oi_colors[1], alpha.f = 0.1))
points(statcast2024$hit_coord_x[statcast2024$hit_location==3], 
     statcast2024$hit_coord_y[statcast2024$hit_location==3],
     pch = 16, cex = 0.2, 
...
Although MLB Stadiums are equipped with lots of very cool camera and ball tracking technology, the variables `hit_coord_x` and `hit_coord_y` are *not* derived from those technologies.
Instead a stringer manually marks the location of the batted ball on a tablet so the coordinates `hit_coord_x` and `hit_coord_y` are expressed in units of pixels on that tablet.
[Jim Albert](https://bayesball.github.io), who is one of the founding fathers of Statistical research in baseball, [suggested](https://baseballwithr.wordpress.com/2021/04/) the following transformation from the original `(hit_coord_x, hit_coord_y)` coordinate system to one that places home plate at the bottom of the plot (with coordinates `(0,0)`) and measures distances in feet.
$$
\begin{align}
x &= 2.5 \times (\texttt{hit\_coord\_x} - 125.42) & y &= 2.5 \times (198.27 - \texttt{hit\_coord\_y})
\end{align}
$$
...
#| fig-alt: Scatterplot of batted ball locations with yellow foul lines and white basebaths overlaid

statcast2024 <-
  statcast2024 |>
  dplyr::mutate(
    x = 2.5 * (hit_coord_x - 125.42),
    y = 2.5 * (198.27 - hit_coord_y))
...
def_atbat2024 <-
  ...
  dplyr::mutate(
    ...
    end_des = dplyr::last(des)) |>
  dplyr::ungroup() |>
  dplyr::filter(pitch_number == 1) |>
  dplyr::arrange(game_date, game_id, at_bat_number, pitch_number) |>
  dplyr::select(
    game_date, game_id, at_bat_number,
    end_events, end_type, end_des,
    batter_id,
    pitcher_id, fielder_2_id, fielder_3_id,
    fielder_4_id, fielder_5_id, fielder_6_id,
  ...
  dplyr::filter(!is.na(end_events) & !is.na(end_des))
```
...
table(grepl("out", bip$end_des[bip$end_events == "fielders_choice"]))
```
...
which(grepl("out", bip$end_des) & bip$end_events == "fielders_choice")
bip$end_des[which(grepl("out", bip$end_des) & bip$end_events == "fielders_choice")]
```
We will manually change the value of `end_events` in row 65236 to "fielders_choice_out".
...
bip$end_events[65236] <- "fielders_choice_out"
out_events <- 
...
#| fig-alt: Batted ball locations overlaid over a square grid of blue points
...
#| fig-alt: Square grid of points restricted to the dimensions of a baseball diamond
...
#| fig-alt: Heatmap of out probabilities
...
#| fig-height: 6
#| fig-align: center
#| fig-cap: Heatmap of out probabilities fitted with a generalized additive model
#| fig-alt: Heatamp of out probabilities
...
all_preds <-
  predict(object = gam_fit, 
          ...
          type = "response")
def_atbat2024$p_out <- as.vector(all_preds) #<2>
...
```{r}
#| label: find-na
table(def_atbat2024$end_events[is.na(def_atbat2024$p_out)], useNA = "always")
...
save(fit_list, mid_x, mid_y, file = "position_gam_fits.RData") #<12>
```
...
Taking a looking at the largest $\textrm{RAA}_{\ell}^{f}$ values, we see several recent [Golden Glove](https://en.wikipedia.org/wiki/Gold_Glove_Award) winners who are known for their fielding prowess (Santana, Walker, Goldschmidt, Olson).
```{r}
#| label: raa-f3
...
raa_f3 <-
  ...
  dplyr::group_by(fielder_3_id) |>
  dplyr::summarize(RAA_f3 = sum(RAA_f3, na.rm = TRUE)) |>
  dplyr::rename(key_mlbam = fielder_3_id)
...
raa_f1 <-
  ...
  dplyr::rename(key_mlbam = pitcher_id) |>
...
raa_p <-
  def_atbat2024 |>
  dplyr::select(pitcher_id, delta_p) |>
  dplyr::group_by(pitcher_id) |>
...
```{r}
save(raa_f, raa_p, file = "mlb_raa_defensive2024.RData")
```

```{r}
#| label: save-def-atbat
#| echo: false
save(def_atbat2024, file = "mlb_def_atbat2024.RData")
```
...
load("mlb_raa_offensive2024.RData")
...
raa |>
  dplyr::arrange(dplyr::desc(RAA)) |>
  dplyr::select(Name, RAA, RAA_b, RAA_br, RAA_f, RAA_p) |>
  dplyr::slice_head(n = 10)
```
...
all_players <- unique( #<1>
  c(def_atbat2024$batter_id, def_atbat2024$pitcher_id, def_atbat2024$fielder_2_id,
    def_atbat2024$fielder_3_id, def_atbat2024$fielder_4_id, def_atbat2024$fielder_5_id,
    def_atbat2024$fielder_6_id, def_atbat2024$fielder_7_id, def_atbat2024$fielder_8_id, def_atbat2024$fielder_9_id))
pitchers <- unique(def_atbat2024$pitcher_id) #<2>
...
position_pa <-
  def_atbat2024 |>
  dplyr::filter(batter_id %in% position_players) |> #<1>
...
pitcher_pa <-
  def_atbat2024 |>
  dplyr::group_by(pitcher_id) |> #<4>
  dplyr::summarise(n = dplyr::n()) |>
  dplyr::arrange(dplyr::desc(n)) |>
  dplyr::rename(key_mlbam = pitcher_id) #<5>
