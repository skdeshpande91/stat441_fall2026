...
- Too many $\hat{p}_{i}$'s on wrong-side of 50% indicates poor calibration.
...
- $\hat{p} = 0.501$ and $\hat{p} = 0.999$ have same loss when $y = 0$

## Other Losses


- Brier Score & log-loss penalize distance b/w forecast $\hat{p}$ & $Y$
$$
\text{Brier} = n^{-1}\sum_{i = 1}^{n}{(y_{i} - \hat{p}_{i})^2}.
$$

$$
\textrm{LogLoss} = -1 \times n^{-1}\sum_{i = 1}^{n}{\left[ y_{i} \times \log(\hat{p}_{i}) + (1 - y_{i})\times\log(1-\hat{p}_{i})\right]}.
$$
...
- Form projects groups by tomorrow
  ...
  - Sign-up for teams [here](https://docs.google.com/spreadsheets/d/1aGQ9m2OKLRnTq11pLawBTwF8oxFPIaRmYWvD_8kGhoo/edit?gid=0#gid=0)
